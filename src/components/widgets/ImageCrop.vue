...
export default {
  name: 'image-crop',
  props: ['page', 'startcb', 'cropcb', 'ratio'],
  data() {
    return {
      imageWidth: 0,
      imageHeight: 0,
      ...
      this.$nextTick(() => {
        ...
        $(vm.$els.imgcrop).Jcrop({
          ...
          vm.jcropapi.setSelect([
            vm.page.getIn(['crop', 'l']),
            vm.page.getIn(['crop', 't']),
            vm.page.getIn(['crop', 'l']) + vm.page.getIn(['crop', 'w']),
            vm.page.getIn(['crop', 't']) + vm.page.getIn(['crop', 'h'])
          ])
        })
      })
    },
